...
| purchase(options:) | https://developer.apple.com/documentation/storekit/product/purchase(options:) | Platform table doesn't include visionOS |
...
| Statements — Switching Over Future Enumeration Cases (The Swift Programming Language) | https://docs.swift.org/swift-book/documentation/the-swift-programming-language/statements/#Switching-Over-Future-Enumeration-Cases | `@unknown default` semantics; omitting it is an error in Swift 6 |

## Verification provenance for the bridge/skeleton

- The bridge/skeleton in this skill typechecks clean under `swiftc -swift-version 6 -typecheck` (Swift 6.3.2 / Xcode 26.5), 0 errors/warnings. *Compiled-verified.*
- `Transaction.updates`, `.currentEntitlements`, `.finish()`, `.revocationDate`, `AppStore.sync()`, `Product.products(for:)`, `.purchase(options:)`, `.PurchaseResult`, `VerificationResult` — each symbol's existence/signature confirmed against Apple's StoreKit documentation (fetched per CONTRIBUTING.md's doc-JSON tip). *Apple-doc-verified.*
