...
description: Generate a single-file HTML visual design mockup for an iOS app from a written spec, in the style of a designer's user-flow canvas. Use this whenever the user has an iOS app spec (PRD, requirements doc, feature description, user stories) and wants to see what it looks like — including phrases like "turn this spec into a mockup", "show me the screens", "design this", "make a prototype", "visualize this app", or "make a Figma-like flow". Also trigger when the user wants to communicate iOS designs to non-engineers (PMs, stakeholders, designers) without building a working prototype. The output is a single HTML file with iPhone frames arranged on a canvas, SVG arrows showing navigation, and a complete design tokens panel — not a clickable prototype, not real Figma files. Do NOT trigger when the user is discussing iOS screens or flows conceptually (planning, asking questions, reviewing existing screens), asking for SwiftUI or production code, or has not asked for a visual artifact to be generated.
---
