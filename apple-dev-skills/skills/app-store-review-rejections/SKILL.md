...
|---|---|---|
| **2.1 App Completeness** | Crash on reviewer's device/OS, dead-end flow, placeholder content, ad fails to load → blank space, debug/test ad shown in the prod build | Run the build on a *clean* device + the oldest supported OS. Ensure ads degrade gracefully (no empty frame on no-fill). Release and TestFlight should read the **same production** ad unit ID — don't switch to a Google test ad unit ID for TestFlight, that's a Google policy violation risk (invalid traffic) if it ever slips into a public release; instead register each tester's device as a Google test device (AdMob console → Test devices) so it gets test ads on the prod ID. Verify ads actually render, not a blank no-fill frame. No `<TRANSLATE>` / lorem strings. |
