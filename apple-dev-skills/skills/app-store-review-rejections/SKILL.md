...
| **2.5.x / export compliance** | Build held "Processing" pending the encryption/export-compliance answer | Answer export compliance in ASC (uses standard crypto only) — user-owned — a build sits in "Processing" until a human answers it, so it is a release-day blocker, not an automation step (`asc-api-automation` covers what the API *can* drive). |
